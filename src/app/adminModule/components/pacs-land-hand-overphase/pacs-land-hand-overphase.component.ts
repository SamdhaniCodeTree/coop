...
@Component({
  selector: 'app-pacs-land-hand-overphase',
  templateUrl: './pacs-land-hand-overphase.component.html',
  styleUrls: ['./pacs-land-hand-overphase.component.css']
})
export class PacsLandHandOverphaseComponent implements OnInit {
    ...
    try {
      if (this.validate()) {
        ...
        if (response.success) {
          ...
          //   this.router.navigate(['/jc/PacsLandAllotmentphase2']);
          // }
          // else{
  ...
  async onPositionStatementChange(event: any): Promise<void> {
    try {
      ...
      if (event.target.files.length > 0) {
        ...
        let  file1 = file.replace('data:application/pdf;base64,','');
          this.LandAllocateData.positionStatementPdf = file1;
        } else {
