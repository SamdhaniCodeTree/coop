...
@Component({
  selector: 'app-pacs-landallotmentphase-three-ins',
  templateUrl: './pacs-landallotmentphase-three-ins.component.html',
  styleUrls: ['./pacs-landallotmentphase-three-ins.component.css']
})
export class PacsLandallotmentphaseThreeInsComponent implements OnInit {
      ...
      try {
        ...
        if (response.success) {
         ...
         // this.router.navigate(['/jc/pacsLandHandOverphase2']);
          this.router.navigate(['/jc/pacsLandHandOverphase3'], {
    ...
    try {
      ...
      if (event.target.files.length > 0) {
        ...
        if (response) 
        {
          ...
          console.log(this.LandAllocateData.titleDeedPhotoUpload);
          debugger;
        } else {
          this.ngxToaster.warning('file is Empty !!!, Please try again.');
          event.target.value = '';
