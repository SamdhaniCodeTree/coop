...
@Component({
  selector: 'app-apcob-dgm-file-detailspending',
  templateUrl: './apcob-dgm-file-detailspending.component.html',
  styleUrls: ['./apcob-dgm-file-detailspending.component.css']
})
export class ApcobDgmFileDetailspendingComponent implements OnInit {
 ...
 reportTotals = {
  ...
  total_paid_amount: '-',
  balance_amount: '-', 
  payment_recommendation_file_by_apcob: '-',
}
     ...
     try { 
        ...
        //const response = await this.inspectionAPI.TechManagerGet(req);
         ...
         if (response.success) { 
             ...
             for (let i = 0; i < this.DEVICEDETAILSLIST.length; i++) {
              // tslint:disable-next-line: radix
              this.reportTotals.transaction_id += parseInt(
                this.DEVICEDETAILSLIST[i].TRANSACTION_ID
              );
              this.reportTotals.transaction_date += parseInt(
                this.DEVICEDETAILSLIST[i].TRANSACTION_DATE
              );
              this.reportTotals.pacs_count += parseInt(
                this.DEVICEDETAILSLIST[i].PACS_COUNT
              );
              this.reportTotals.requested_amount += parseInt(
                this.DEVICEDETAILSLIST[i].REQUESTED_AMOUNT
              );
              this.reportTotals.transaction_status_msg += parseInt(
                this.DEVICEDETAILSLIST[i].TRANSACTION_STATUS_MSG
              );
              this.reportTotals.last_payment_date += parseInt(
                this.DEVICEDETAILSLIST[i].LAST_PAYMENT_DATE
              );
              this.reportTotals.total_paid_amount += parseInt(
                this.DEVICEDETAILSLIST[i].TOTAL_PAID_AMOUNT
              );
              this.reportTotals.balance_amount += parseInt(
                this.DEVICEDETAILSLIST[i].BALANCE_AMOUNT
              );
              const singleRow = {
                S_NO: i + 1,
                transaction_id: this.DEVICEDETAILSLIST[i].TRANSACTION_ID,
                transaction_date: this.DEVICEDETAILSLIST[i].TRANSACTION_DATE,
                pacs_count: this.DEVICEDETAILSLIST[i].PACS_COUNT,
                requested_amount: this.DEVICEDETAILSLIST[i].REQUESTED_AMOUNT,
                transaction_status_msg: this.DEVICEDETAILSLIST[i].TRANSACTION_STATUS_MSG,
                last_payment_date: this.DEVICEDETAILSLIST[i].LAST_PAYMENT_DATE,
                total_paid_amount: this.DEVICEDETAILSLIST[i].TOTAL_PAID_AMOUNT,
                balance_amount: this.DEVICEDETAILSLIST[i].BALANCE_AMOUNT,
