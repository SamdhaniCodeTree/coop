...
@Component({
  selector: 'app-internet-aprove-reject',
  templateUrl: './internet-aprove-reject.component.html',
  styleUrls: ['./internet-aprove-reject.component.css']
})
export class InternetAproveRejectComponent implements OnInit {
...
debugger;
  ...
  async ApprovedInternetDetails(obj:any){
    ...
    try {
      if (confirm('are you sure want to Approved Device Received Status & Calibration ??')) {
        ...
        const req = {
          ...
          pacs_code: this.packcode,
          supervisor_file_path:obj

        };
        debugger;
        this.spinner.show();
        const response = await this.ceoAPI.InternetSuperviserfilepath(req);
        this.spinner.hide();
        if (response.success) {
  ...
  async btnPDF(): Promise<void> {
    ...
    try {
      ...
      debugger;
      const fileName = 'Internet Details';
      let basePDF = '';
      this.spinner.show();

      const res = await this.ceoAPI.CeoInternetDtailsCert(req);
      if (res.success) {
        basePDF = res.result;
        this.CertifcateFile=basePDF;
        this.ApprovedInternetDetails(basePDF);
