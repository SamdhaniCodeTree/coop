...
@Component({
  selector: 'app-mli-mandal',
  templateUrl: './mli-mandal.component.html',
  styleUrls: ['./mli-mandal.component.css'],
})
export class MliMandalComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async loadReport(): Promise<void> {
    ...
    try {
      ...
      if (response.success) {
        ...
        //  this.mandalName=response.MANDAL_NAME[0];
        this.excelData = [];
        debugger;
...
debugger;
  ...
  async viewPDF(path: string): Promise<void> {
    try {
      ...
      //await this.utils.viewJPVPDF(path);
    } catch (error) {
      this.spinner.hide();
      this.utils.catchResponse(error);
    }
  ...
  async viewPDFfile(path: string): Promise<void> {
    try {
      const req = {
        filePath: path,
      };
