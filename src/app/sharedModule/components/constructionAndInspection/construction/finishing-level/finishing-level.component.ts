...
@Component({
  selector: 'app-finishing-level',
  templateUrl: './finishing-level.component.html',
  styleUrls: ['./finishing-level.component.css'],
})
export class FinishingLevelComponent implements OnInit {
  status = 0;
  statusMessage = '';
  dataAvailable = false;
  maxDate!: Date;
  ...
  async onVillageChange(): Promise<void> {
    try {
      ...
      this.spinner.show();
      const response = await this.dllicAPI.finishingDetailsById(req);
      this.spinner.hide();
      if (response.success) {
...
debugger;
      ...
      const response = await this.dllicAPI.finishingSub(this.constructionReq);
      this.spinner.hide();
      if (response.success) {
